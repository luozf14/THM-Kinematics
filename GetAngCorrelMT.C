...
void testMT()
{
    ...
    vecSpectatorDec.SetPhi(TMath::Pi());

    auto workItem = [](UInt_t workerID)
    {
        TFile f(Form("myFile_%u.root", workerID), "RECREATE");

        TH2D *h2EbVsAngb = new TH2D("h2EbVsAngb", "E_{b} vs #theta_{b}", 200, 0, 180, 200, 0, 25);
        h2EbVsAngb->SetXTitle("#theta_{b} [deg]");
        h2EbVsAngb->SetYTitle("E_{b} [MeV]");
        TH2D *h2EBVsAngB = new TH2D("h2EBVsAngB", "E_{B} vs #theta_{B}", 200, 0, 35, 200, 0, 2);
        h2EBVsAngB->SetXTitle("#theta_{B} [deg]");
        h2EBVsAngB->SetYTitle("E_{B} [MeV/u]");
        TH2D *h2EsVsAngs = new TH2D("h2EsVsAngs", "E_{n} vs #theta_{n}", 200, 0, 180, 200, 0, 5);
        h2EsVsAngs->SetXTitle("#theta_{n} [deg]");
        h2EsVsAngs->SetYTitle("E_{n} [MeV]");

        TH2D *h2AngBVsAngb = new TH2D("h2AngBVsAngb", "#theta_{B} vs #theta_{b}", 200, 0, 180, 200, 0, 35);
        h2AngBVsAngb->SetXTitle("#theta_{b} [deg]");
        h2AngBVsAngb->SetYTitle("#theta_{B} [deg]");
        TH2D *h2AngBVsAngs = new TH2D("h2AngBVsAngs", "#theta_{B} vs #theta_{n}", 200, 0, 180, 200, 0, 35);
        h2AngBVsAngs->SetXTitle("#theta_{n} [deg]");
        h2AngBVsAngs->SetYTitle("#theta_{B} [deg]");
        TH2D *h2AngbVsAngs = new TH2D("h2AngbVsAngs", "#theta_{b} vs #theta_{n}", 200, 0, 180, 200, 0, 180);
        h2AngbVsAngs->SetXTitle("#theta_{n} [deg]");
        h2AngbVsAngs->SetYTitle("#theta_{b} [deg]");
        ...
        TH1D *h1ExA = new TH1D("h1ExA", "ExA", 1000, 0, 10);
        h1ExA->SetXTitle("ExA [MeV]");

        TH1D *h1ExA_sub = new TH1D("h1ExA_sub", "ExA", 1000, 0, 10);
        ...
        for (UInt_t i = 0; i < nNumbers; ++i)
        {
            ...
            TVector3 heavyLocal = (SlitDistance / std::cos(pB_l.Angle(vecMdm))) * pB_l.Unit() - vecMdm;
            heavyLocal.RotateY(-MdmAng);
            Bool_t heavyDet = std::abs(theta_B - MdmAng * 180. / TMath::Pi()) < 2. && std::abs(heavyLocal.X()) < SlitWidth / 2. && std::abs(heavyLocal.Y()) < SlitWidth / 2.;

            TVector3 lightLocal = (SiDistance / std::cos(pb_l.Angle(vecSi))) * pb_l.Unit() - vecSi;
            lightLocal.RotateY(SiAng);
            Bool_t lightDet = std::abs(theta_b - SiAng * 180. / TMath::Pi()) < 20. && std::abs(lightLocal.X()) < SiWidth / 2. && std::abs(lightLocal.Y()) < SiWidth / 2.;
